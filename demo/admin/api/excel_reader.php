...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
                        $out .= '<th><input type="checkbox" onchange="selectall(this)" name="selectall"></th>';
			if ($row_numbers) {
				$out .= "\n\t\t<th>&nbsp</th>";
			}
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			$out .= "\n\t<tr".' id="book'.$row.'"'." style=\"$style\">";
			$out .= '<th><input class="bookselect" type="checkbox" name="bookselect"></th>';
			if ($row_numbers) {
				$out .= "\n\t\t<th>$row</th>";
			}
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$out .= "<nobr class=".'"bookdata"'.">".nl2br($val)."</nobr>";
					$out .= "</td>";
				}
			}
			$out .= "</tr>\n";
