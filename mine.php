...
{
    case 'GET':
    if((isset($_GET["email"]))&&(isset($_GET["token"]))&&(isset($_GET["type"])))
    {
        $type=$_GET["type"];
        $email=$_GET["email"];
	    $confirmation=$_GET["token"];
	    $query = sprintf("SELECT * FROM `book` WHERE email='%s' and status='%s'",mysql_real_escape_string($email),mysql_real_escape_string($type));
